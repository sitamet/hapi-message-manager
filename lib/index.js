...
function MessageManager(broker, options) {
    ...
    function Routing(message, messageContent) {
        ...
        let content = lodash(messageContent);
        ...
        return {
            ...
            reply: function (err, result, next) {
                send({ err, result, next, getSuccessKey: this.keyToDone, getErrorKey: this.keyToError });
            },

            send: function (err, result, next) {
                send({ err, result, next, getSuccessKey: this.key, getErrorKey: this.keyToError });
            }
        }
    }
    ...
    function publish(content, key, next) {
        ...
        broker.publish(publication, content, key, (err, publication) => {
